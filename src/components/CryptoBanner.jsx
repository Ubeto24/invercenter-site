...
export default function CryptoBanner() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // CoinGecko requiere API key. Soportamos demo y pro.
        const demoKey = import.meta.env.VITE_CG_DEMO_API_KEY;
        const proKey = import.meta.env.VITE_COINGECKO_API_KEY;
        const headers = { Accept: "application/json" };
        if (proKey) headers["x-cg-pro-api-key"] = proKey;
        else if (demoKey) headers["x-cg-demo-api-key"] = demoKey;

        const res = await fetch(API, { cache: "no-store", mode: "cors", headers });
        ...
        if (mounted) {
          const hasAnyKey = !!(import.meta.env.VITE_CG_DEMO_API_KEY || import.meta.env.VITE_COINGECKO_API_KEY);
          const msg = hasAnyKey
            ? t("crypto.error", "No se pudo cargar datos de mercado.")
            : t("crypto.error", "No se pudo cargar datos de mercado.");
          setError(msg);
        }
        console.error(e);
    ...
    fetchData();
    const id = setInterval(fetchData, 300000); // 5 min para evitar rate limit
    return () => {
      mounted = false;
      clearInterval(id);
    };
  ...
  return (
    <section className="bg-gradient-to-r from-blue-50 to-gray-50 border-y border-blue-200/60 py-3">
      <div className="max-w-7xl mx-auto px-3 md:px-6">
            ...
            {looped.map((c, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-blue-800 font-bold">{c?.current_price != null ? `$${c.current_price.toLocaleString()}` : "N/A"}</span>
                  <span
        ...
        <p className="text-[11px] text-gray-400 mt-1">{t("crypto.disclaimer", "Datos informativos por CoinGecko. No constituyen asesoria financiera.")}</p>
      </div>
    </section>
  );
}
